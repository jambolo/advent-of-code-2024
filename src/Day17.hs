...
updateState state reg value i = State (regs state Array.// [(reg, value)]) i (output state)
...
day17_part2 input = do
    let (registerLines, programLines) = break null $ lines input
    let _ = Array.listArray (0, 2) $ map (\s -> read (s =~ "[0-9]+") :: Int) registerLines :: Registers
    let program = map (\s -> read s :: Int) $ splitOn "," $ dropWhile (not . isDigit) (head $ tail programLines) :: Program
    print program
    let result = head $ filter (\i -> execute (State (Array.listArray (0, 7) [i, 0, 0]) 0 []) program == program) [0..]
    return result
